...
async fn search(params: web::Json<env::SearchParams>) -> Result<HttpResponse> {
    ...
    for dst_lang in &params.dst_langs {
        let result = query_translate(
            &params.original_text,
            &params.src_lang,
            dst_lang,
            translate_api_key.as_ref(),
        )
        .await;
        results.push(result);
    }
...
async fn query_translate(
    ...
    let translation = resp_body_json.get("translations").unwrap().get(0).unwrap();
...
async fn main() -> std::io::Result<()> {
    let listen_addr_port: String = format!("{}:{}", LISTEN_ADDR, LISTEN_PORT);
    println!("Server {} running", listen_addr_port);
